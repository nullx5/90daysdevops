...
export const variables = {
  ...
  frameLifetime: 10,
  startTime: 0,
  gameStartTime: 0, // Nuevo: tiempo de inicio del juego
};

export const assets = Factory.makeAssets(map, variables);

export default function playGame(player, reactRoot, onGameEnd) {
  variables.animationId = requestAnimationFrame(() => playGame(player, reactRoot, onGameEnd));
  const board = document.querySelector("#board");
  const ctx = board.getContext("2d");
  if (variables.start === true) {
    Game.finishSetup(variables, player, null, onGameEnd, assets, ctx);
  }
